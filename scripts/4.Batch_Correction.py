...
snap.tl.umap(dat, n_comps=2, use_rep="X_spectral_mnc", key_added="umap_mnc", random_state=0, inplace=True)
...
# --- 6. Harmony Correction ---
logging.info("Running Harmony batch correction...")
snap.pp.harmony(dat,
                batch=batch_var,
                use_rep='X_spectral',
                max_iter_harmony=20)
# Harmony results are stored in 'X_spectral_harmony' by default

logging.info("Running UMAP on Harmony...")
snap.tl.umap(dat, n_comps=2, use_rep="X_spectral_harmony", key_added="umap_harmony", random_state=0, inplace=True)

# Cluster Harmony
logging.info("Clustering Harmony...")
snap.pp.knn(dat, n_neighbors=50, use_rep='X_spectral_harmony', method='kdtree', inplace=True, random_state=0)
