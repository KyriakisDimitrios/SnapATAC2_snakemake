...
logging.info("Started: Batch_Correction (Stratified Features)")
...
# --- 1. Load to RAM ---
# Features are already selected and masked from Rule 3!
...
# --- 3. Pre-Correction UMAP ---
logging.info("Running UMAP (Pre-correction)...")
snap.tl.umap(dat_mem, n_comps=2, use_rep='X_spectral', key_added='umap', random_state=random_state, inplace=True)
snap.pl.umap(dat_mem, color="sample", use_rep='X_umap', out_file=png_pre, height=500, width=500, show=False)

# --- 4. Harmony Correction (With Sample Imbalance Fix) ---
...
snap.pp.knn(dat_mem, n_neighbors=50, use_rep='X_spectral_harmony', method='kdtree', inplace=True,
            random_state=random_state)
...
snap.pp.knn(dat_mem, n_neighbors=50, use_rep='X_spectral_mnc', method='kdtree', inplace=True, random_state=random_state)
...
# Create flag file so Snakemake knows we succeeded
