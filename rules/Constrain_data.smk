...
rule constrain_meta:
    ...
    output:
        subset = get_res_path(config['metadata']['constrain']['dir'] + "{sample}.h5ad")
    log: get_res_path(config['metadata']['constrain']['log'] + "{sample}.log")
...
rule batch_meta:
    ...
    params:
        dataset = get_res_path(config['metadata']['merge']['AnnDataSet']),
        ...
        png = report(get_res_path(config['metadata']['batch']['dir'] + "eigen.png"),category="CommonSamples",subcategory='BatchCorrection')
    output:
        flag_out = get_res_path(config['metadata']['batch']['flag']),
...
rule cluster_meta:
    input: flag = get_res_path(config['metadata']['batch']['flag'])
    params:
        dataset = get_res_path(config['metadata']['merge']['AnnDataSet']),
        res = config['analysis_params']['clustering']['resolutions'],
        out_dir = get_res_path(config['metadata']['clustering']['dir'])
    output:
...
rule gem_meta:
    input: h5ad = get_res_path(config['metadata']['clustering']['h5ad'])
    params:
        wd = config['projdir'],
