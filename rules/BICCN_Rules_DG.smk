...
rule subset_DG:
    ...
    params:
        ...
        target_subtypes = ",".join(config['branches'][branch_to_run]['subset']['target_subtypes']),
        group_by = config['branches'][branch_to_run]['subset']['group_by'],
        bin_size = config['analysis_params']['tile_qc']['bin_size']
    output:
        # FIX: Use 'h5ad_out' which contains "{sample}_subsetDG.h5ad" defined in config
        output_file = get_path(branch_to_run, "subset_DGsub", "h5ad_out")
    log:
        get_path(branch_to_run, "subset_DGsub", "log")
...
rule merge_DG:
    input:
        adatas = expand(get_path(branch_to_run, "subset_DGsub", "h5ad_out"),sample=config['samples']),
