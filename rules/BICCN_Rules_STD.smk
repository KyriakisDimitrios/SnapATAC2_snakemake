...
rule tile_qc_std:
    ...
    output:
        tiled=get_path(branch_to_run,"tile_qc","h5ad_out"),
        raw_stats=get_path(branch_to_run,"tile_qc","raw_qc")
    log:
        get_path(branch_to_run, "tile_qc", "log")
    conda: '../envs/magic_env.yaml'
    shell:
        """
...
rule feature_qc_std:
    input:
        ...
        adatas = get_path(branch_to_run, "tile_qc", "h5ad_out")
    params:
    ...
    output:
        ...
        features_out = os.path.join(get_path(branch_to_run, "feature_qc", "dir"), "{sample}.h5ad")
    log:
        get_path(branch_to_run, "feature_qc", "log")
    conda: '../envs/magic_env.yaml'
    shell:
        """
...
rule merge_std:
    input:
        adatas = expand(get_path(branch_to_run, "feature_qc", "h5ad_out"),sample=config['samples']),
        metadata_path= config['metadata_path']
    output:
